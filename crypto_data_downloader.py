...
symbols = ["BTCUSDT", "ETHUSDT"]  # BTC/USDT和ETH/USDT交易对
...
# 创建输出目录
output_base_dir = "crypto_data"
os.makedirs(output_base_dir, exist_ok=True)

# 为每个加密货币创建子目录
for symbol in symbols:
    symbol_dir = os.path.join(output_base_dir, symbol.lower().replace("usdt", ""))
...
def get_historical_data(symbol, start, end, interval_type, session=None):
    if session is None:
...
# 函数：下载指定加密货币和时间间隔的所有历史数据
def download_all_historical_data(symbol, interval_type):
    # 从符号中提取货币名称 (例如 BTCUSDT -> btc)
    currency = symbol.lower().replace("usdt", "")
    symbol_dir = os.path.join(output_base_dir, currency)
    
    print(f"\n开始下载 {symbol} {interval_type} 数据...")
    
    # 文件名格式：BTC_4h.csv, BTC_1d.csv, BTC_1w.csv
    filename = os.path.join(symbol_dir, f"{currency.upper()}_{interval_type}.csv")
    ...
    while current_start_time < end_time:
        data = get_historical_data(symbol, current_start_time, end_time, interval_type, session)
        if not data:
            print(f"未获取到 {symbol} {interval_type} 数据，可能已经到达最新数据或API限制")
            break
        ...
        print(f"已拉取 {symbol} {interval_type} 数据 {len(data)} 条 (总计: {current_count})，当前Close Time：{datetime_str}")
        ...
        if current_count % 5000 == 0:
            print(f"已下载 {current_count} 条 {symbol} {interval_type} 数据，临时保存...")
            ...
            temp_filename = os.path.join(symbol_dir, f"{currency.upper()}_{interval_type}_temp.csv")
    ...
    if all_data:
        ...
        if os.path.exists(filename) and last_time is not None:
            try:
                ...
                print(f"{symbol} {interval_type} 数据已更新，总计 {len(merged_df)} 条记录")
            ...
            # 新文件，直接保存
            df.to_csv(filename, index=False)
            print(f"{symbol} {interval_type} 数据已保存，总计 {len(df)} 条记录")
        
        # 创建兼容版本（用于回测系统）
        create_compatible_csv(df, interval_type, symbol)
        ...
        print(f"{symbol} {interval_type} 没有新数据需要更新")
        return 0

def create_compatible_csv(df, interval_type, symbol):
    """创建兼容回测系统的CSV文件格式"""
    try:
        # 复制数据框
        ...
        # 保存兼容版本
        compatible_filename = os.path.join(symbol_dir, f"{currency.upper()}_{interval_type}_compatible.csv")
...
# 恢复下载功能
def resume_download(symbol, interval_type):
    """从临时文件恢复下载"""
    currency = symbol.lower().replace("usdt", "")
    symbol_dir = os.path.join(output_base_dir, currency)
    temp_filename = os.path.join(symbol_dir, f"{currency.upper()}_{interval_type}_temp.csv")
    ...
    if os.path.exists(temp_filename):
        try:
            ...
            if last_close_time:
                print(f"发现 {symbol} {interval_type} 的临时数据，从 {datetime.datetime.fromtimestamp(last_close_time/1000).strftime('%Y-%m-%d %H:%M:%S')} 继续下载")
...
def main():
    ...
    for symbol in symbols:
        symbol_records = 0
        currency = symbol.lower().replace("usdt", "")
        crypto_stats[currency] = {"intervals": {}}
        ...
        for interval in intervals:
            # 检查是否有临时文件可以恢复
            resume_point = resume_download(symbol, interval)
            if resume_point:
                # 恢复下载的逻辑已在download_all_historical_data中处理
                pass
            
            records = download_all_historical_data(symbol, interval)
            symbol_records += records
            crypto_stats[currency]["intervals"][interval] = records
        ...
        crypto_stats[currency]["total"] = symbol_records
        
        # 在下载不同货币之间添加较长的延迟
        if symbol != symbols[-1]:  # 不是最后一个货币
            delay = random.uniform(8.0, 15.0)
            print(f"\n完成 {symbol} 所有数据下载，等待 {delay:.1f} 秒后继续下一个加密货币...")
            time.sleep(delay)
    ...
    for symbol in symbols:
        ...
        print(f"\n{currency.upper()} 数据文件:")
        for interval in intervals:
            print(f"  • {os.path.join(symbol_dir, f'{currency.upper()}_{interval}.csv')} (原始格式)")
            print(f"  • {os.path.join(symbol_dir, f'{currency.upper()}_{interval}_compatible.csv')} (回测兼容格式)")
