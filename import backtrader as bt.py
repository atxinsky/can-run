...
class MACDStrategy(bt.Strategy):
    ...
    params = (
        ...
        ("period_signal", 9),  # 信号线
        ("atr_period", 14),  # ATR周期
        ("stop_loss_atr", 1.5),  # 止损倍数（以ATR为单位）
        ("take_profit_atr", 3.0),  # 止盈倍数（以ATR为单位）
        ("leverage", 6),  # 杠杆倍数
        ("trade_percent", 0.4),  # 每次交易使用总资金的40%
    ...
    def __init__(self):
        ...
        self.atr = bt.indicators.AverageTrueRange(self.data, period=self.params.atr_period)  # ATR计算
        self.order = None
    ...
    def next(self):
        ...
        if self.macd.macd[0] > 0 and self.crossup[0]:
            ...
            stop_loss = self.data.close[0] - self.params.stop_loss_atr * atr_value  # 动态止损
            take_profit = self.data.close[0] + self.params.take_profit_atr * atr_value  # 动态止盈

            # 设置止损止盈
            self.sell(price=stop_loss, exectype=bt.Order.Stop, parent=self.position)
            self.sell(price=take_profit, exectype=bt.Order.Limit, parent=self.position)

        # 判断是否满足平仓条件（死叉情况下退出）
        if self.position and self.crossdown[0]:
            self.close()  # 死叉平仓
...
def run_backtest(start_date, end_date):
    ...
    data = bt.feeds.GenericCSVData(
        ...
        dtformat=('%Y-%m-%d %H:%M:%S'),  # 修改时间格式，适配你的数据
        datetime=0,
        open=1,
        high=2,
        low=3,
